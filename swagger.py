...
@app.get("/order_reference/{order_id}/{sales_order_id}",
         response_model=OrderReference)
def get_order_reference(order_id: int, sales_order_id: int):
    order_reference = create_order_reference(order_id, sales_order_id,
                                             DATABASE)

    if isinstance(order_reference, dict) and "error" in order_reference:
        raise HTTPException(
            status_code=400, detail=order_reference["error"]
        )
...
def create_order_reference(
    order_id: int, sales_order_id: int, database: set
) -> Union[OrderReference, dict]:
...
@app.put("/create_customer_party/{despatch_id}/"
         "{customer_assigned_account_id}/"
         "{supplier_assigned_account_id}")
def create_customer_party(
    despatch_id: str, customer_assigned_account_id: str,
    ...
    url = f'https://api.example.com/v1/despatch/\
        {despatch_id}/createCustomerParty'
    ...
    try:
        ...
        raise HTTPException(
            status_code=response.status_code, detail=f"HTTP Error: {http_err}"
        )
    except requests.exceptions.RequestException as req_err:
        raise HTTPException(
            status_code=500, detail=f"Request Error: {req_err}"
        )
...
def create_supplier_section(despatch_id: str, sales_order_id: int):
    if sales_order_id not in DATABASE:
        raise HTTPException(
            status_code=404,
            detail="404 - SalesOrderID does not exist in database"
        )
    ...
    url = f'https://api.example.com/v1/\
        despatch/{despatch_id}/createSupplierSection'
...
def create_despatch_line(despatch_id: str,
                         delivered_quantity: int, quantity: int):
    backorder_quantity = quantity - delivered_quantity
    line_status_code = "Completed" if backorder_quantity == 0 else "Revised"
    backorder_reason = "" \
        "" if backorder_quantity == 0 else "User-specified reason"
...
def check_stock(order_id: str):
    if order_id not in ORDERS_DB:
        raise HTTPException(
            status_code=404, detail="Stock data missing."
        )
    order = ORDERS_DB[order_id]
    stock_status = []
    for item in order["items"]:
        available_quantity = STOCK_DB.get(item["item_id"], 0)
        status = "Available" if item["quantity"]\
            <= available_quantity else "Insufficient Stock"
        stock_status.append({
...
def create_despatch_advice(despatch: DespatchAdvice):
    ...
    if despatch.order_id in DESPATCH_DB:
        raise HTTPException(
            status_code=409, detail="Despatch Advice already exists."
        )
    DESPATCH_DB[despatch_id] = despatch.dict()
    return {
        "despatch_id": despatch_id,
        "status": "Initiated",
        "xml_link": f"/despatch/{despatch_id}",
    }
...
def validate_despatch(despatchId: str):
    if despatchId not in DESPATCH_DB:
        raise HTTPException(
            status_code=404, detail="No Despatch Advice found."
        )
...
def retrieve_despatch_xml(despatchId: str):
    ...
    return f"""<DespatchAdvice>
    <cbc:ID>{despatchId}</cbc:ID>
    <cbc:IssueDate>{datetime.now().date()}</cbc:IssueDate>
</DespatchAdvice>"""
...
def update_postal_address(
    despatchId: int, salesOrderId: int, address: DeliveryAddress
):
    if despatchId not in DATABASE or salesOrderId not in DATABASE:
        raise HTTPException(
            status_code=404, detail="SalesOrderId or DespatchId invalid"
        )
    return {
        "despatch_id": despatchId,
        "status": "Postal Address Updated",
    }


@app.put("/v1/despatch/{despatchId}/supplierParty/partytaxScheme")
def update_party_tax_scheme(
    despatchId: int, salesOrderId: int, tax_scheme: PartyTaxScheme
):
    if despatchId not in DATABASE or salesOrderId not in DATABASE:
        raise HTTPException(
            status_code=404, detail="SalesOrderId or DespatchId invalid"
    ...
    return {
        "despatch_id": despatchId,
        "status": "Party Tax Scheme Updated",
    }


@app.put("/v1/despatch/{despatchId}/supplierParty/contacts")
def update_contact_details(
    despatchId: int, salesOrderId: int, contacts: ContactDetails
):
    if despatchId not in DATABASE or salesOrderId not in DATABASE:
        raise HTTPException(
            status_code=404, detail="SalesOrderId or DespatchId invalid"
    ...
    return {
        "despatch_id": despatchId,
        "status": "Contact Details Updated",
    }


@app.put("/v1/despatch/{despatchId}/customerParty/postalAddress")
def update_customer_postal_address(
    despatchId: int, address: DeliveryAddress
):
    if despatchId not in DATABASE:
        raise HTTPException(
            status_code=404, detail="DespatchId invalid"
        )
    return {
        "despatch_id": despatchId,
        "status": "Customer Postal Address Updated",
    }


@app.put("/v1/despatch/{despatchId}/customerParty/partytaxScheme")
def update_customer_party_tax_scheme(
    despatchId: int, tax_scheme: PartyTaxScheme
):
    if despatchId not in DATABASE:
        raise HTTPException(
            status_code=404, detail="DespatchId invalid"
...
def update_customer_contact_details(
    ...
    return {
        "despatch_id": despatchId,
        "status": "Customer Contact Details Updated",
    }
...
def update_order_line_reference(despatchId: int):
    ...
    return {
        "despatch_id": despatchId,
        "status": "Order Line Reference Updated",
    }
...
def get_all_despatch_lines(despatchId: int):
    ...
    return {
        "despatch_id": despatchId,
        "status": "Despatch Lines Retrieved",
    }
...
def get_despatch_line(despatchId: int, Id: int):
    ...
    return {
        "despatch_id": despatchId,
        "despatch_line_id": Id,
        "status": "Despatch Line Retrieved",
    }
...
def finalize_despatch(despatchId: int, request: FinalizeRequest):
    if despatchId not in DATABASE:
        raise HTTPException(
            status_code=404,
            detail="No Despatch Advice found with the given ID",
        )
    return {
        "despatch_id": despatchId,
        "status": "Finalized",
        "final_xml_link": f"/despatch/{despatchId}/final",
    }


@app.post("/v1/despatch/{despatchId}/generatePDF")
def generate_pdf_finalized(despatchId: int):
    if despatchId not in DATABASE:
        raise HTTPException(
            status_code=404,
            detail="No finalized Despatch Advice found",
        )
    return {
        "despatch_id": despatchId,
        "pdf_download_link": f"/despatch/{despatchId}/pdf",
    }
...
def send_documents(despatchId: int):
    ...
    return {
        "despatch_id": despatchId,
        "emailStatus": "STATUS_SUCCESS(200)",
    }
