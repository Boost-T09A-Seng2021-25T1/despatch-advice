...
from src.mongodb import getOrderInfo, addOrder, dbConnect
...
def addDespatchAdvice(data):
    """Add a despatch advice document to the database"""
    try:
        client, db = dbConnect()
        result = addOrder(data, db)
        client.close()
        return result
...
def getDespatchAdvice(despatchId):
    """Get a despatch advice document by ID"""
    try:
        client, db = dbConnect()
        result = getOrderInfo(despatchId, db)
        client.close()
        return result
    except Exception as error:
...
def create_despatch_advice(event_body):
    """
    ...
    try:
        ...
        if "order_id" not in body:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Invalid request format: missing order_id"})
            }
        ...
        inserted_id = addDespatchAdvice(despatch_data)
        
        if not inserted_id:
            return {
                "statusCode": 500,
                "body": json.dumps({"error": "Failed to create despatch advice"})
            }
...
def validate_despatch_advice(despatch_id):
    """
    ...
    """
    try:
        despatch = getDespatchAdvice(despatch_id)
        ...
        try:
            ...
            for xpath in required_elements:
                if not root.findall(xpath, namespaces):
                    ...
                    validation_issues.append(f"Missing required element: {element_name}")
            
            warnings = []
            for xpath in recommended_elements:
                if not root.findall(xpath, namespaces):
                    element_name = xpath.split(":")[-1]
                    warnings.append(f"Missing recommended element: {element_name}")
...
def get_despatch_xml(despatch_id):
    """
...
def update_despatch_advice(despatch_id, event_body):
    """
    ...
    try:
        ...
        if "xml" not in body:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Invalid request format: missing xml field"})
            }
