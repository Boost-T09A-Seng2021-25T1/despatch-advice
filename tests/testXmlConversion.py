...
sample_order_xml = f"""<?xml version="1.0" encoding="UTF-8"?>
...
sample_despatch_xml = f"""<?xml version="1.0" encoding="UTF-8"?>
    <DespatchAdvice xmlns="{despatchSchema}"
        xmlns:cbc="{cbcSchema}"
        xmlns:cac="{cacSchema}">
...
l1 = "<cbc:DespatchAdviceTypeCode>delivery</cbc:DespatchAdviceTypeCode>"
...
class TestXMLConversion(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_order_xml = sample_order_xml

        self.sample_despatch_xml = sample_despatch_xml
    ...
    @patch('src.xmlConversion.cbcSchema', f"{cbcSchema}")
    @patch('src.xmlConversion.cacSchema', f"{cacSchema}")
    ...
    def test_xml_to_json_despatch(self):
        ...
        self.assertEqual(
            result["UUID"], "660e8400-e29b-41d4-a716-446655440001"
        )
        ...
        # Check that Items array exists but is empty 
        # (no OrderLine in despatch XML)
