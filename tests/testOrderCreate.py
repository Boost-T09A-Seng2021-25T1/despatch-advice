...
class TestOrderCreate(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_validate_order_document_valid_json(self):
        is_valid, issues, document = validate_order_document(self.valid_order_json)
        
        self.assertTrue(is_valid)
        self.assertEqual(len(issues), 0)
        self.assertEqual(document, self.valid_order_json)

    @patch('src.orderCreate.xml_to_json')
    async def test_validate_order_document_valid_xml(self, mock_xml_to_json):
        ...
        is_valid, issues, document = validate_order_document(self.valid_order_xml, "xml")
    ...
    async def test_validate_order_document_missing_required_fields(self):
        ...
        is_valid, issues, document = validate_order_document(invalid_order)
    ...
    @patch('src.orderCreate.dbConnect')
    @patch('src.orderCreate.addOrder')
    @patch('uuid.uuid4')
    async def test_create_order_success(self, mock_uuid, mock_add_order, mock_db_connect):
        ...
        result = create_order(self.valid_event_body)
    ...
    async def test_create_order_invalid_input(self, mock_add_order, mock_db_connect):
        ...
        result = create_order(invalid_event_body)
    ...
    async def test_validate_order_not_found(self, mock_get_order, mock_db_connect):
        ...
        result = validate_order("ORD-NOTFOUND")
